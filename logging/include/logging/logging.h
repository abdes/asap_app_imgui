...
private:
   ...
   * and have to continue using that sink for the rest of their lifetime. The
   * Registry class offer an extension feature that allows to dynamically switch
   * the current sink. See Registry::PushSink() and Registry::PopSink().
   *
...
/// \see https://github.com/gabime/spdlog/wiki/3.-Custom-formatting
constexpr const char *DEFAULT_LOG_FORMAT =
    "[%Y-%m-%d %T.%e] [%t] [%^%l%$] [%n] %v";
...
/*!
 * \brief A logging sink implementation that delegates all its logging calls to
 * an encapsulated delegate.
 *
 ...
 * is created:
 *
 * - If the application starts logging early to console and then later needs to
 * log to some different sink after the proper resources for that sink have been
 * initialized (e.g. GUI),
 *
 ...
 * - manage a stack of sinks where the current sink can be temporarily swapped
 * with another sink, to be restored later
 *
 * The Registry creates a default sink at startup to be used by all registered
 * loggers, until an explicit call to PushSink() is made. The default sink is a
 * console logger (color).
 *
 * Example:
 * ```
 * {
...
private:
  ...
  /// The delegating sink used internally to enable swapping of sinks at
  /// runtime.
...
#ifndef NDEBUG
auto ASAP_LOGGING_API FormatFileAndLine(char const *file, char const *line)
    -> std::string;
#define INTERNAL_LOG_PREFIX                                                    \
  asap::logging::FormatFileAndLine(__FILE__, INTERNAL_LINE_STRING)
#else
#define INTERNAL_LOG_PREFIX " "
#endif // NDEBUG
...
#define INTERNAL_ASLOG_3(LOGGER, LEVEL, MSG)                                   \
  LOGGER.LEVEL("{}" MSG, INTERNAL_LOG_PREFIX)
...
#define ASAP_GET_MISC_LOGGER()                                                 \
  asap::logging::Registry::instance().GetLogger("misc")
...
#define ASLOG_MISC(LEVEL, ...)                                                 \
  ASLOG_TO_LOGGER(ASAP_GET_MISC_LOGGER(), LEVEL, __VA_ARGS__)
