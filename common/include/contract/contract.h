...
struct ASAP_COMMON_API Violation {
  ...
  const char *function;
  /// The type of the violation (`precondition`, `postcondition` or `assertion`).
  const char *type;
...
/*!
 * \brief Interface for a violation handler which can switch its implementation at runtime.
 *
 * We expect to have a single violation handler instance in the system, although its iomplementation
 * can be changed at runtime. Therefore, the interface clearly deletes the copy constructors and
...
public:
   ...
   *
   * This method is called when a contract violation occurs. The information related to the
   * violation is provided by the `violation` parameter.
   *
   * \note This function *may* never return.
   *
   * \param violation violation details. Will never be null and is allocated on the stack.
   */
  virtual void HandleViolation(const Violation *violation) = 0;

  /*!
   * \brief Swap the existing violation handler implementation with the give one.
   *
   * \param other_handler at the call time this holds the new violation handler implementation
   * function. Upon return, it will hold the old violation handler function.
   */
 ...
 * to exclusively call it every time the violation handler is needed instead of storing the returned
 * reference.
 *
 ...
 *
 * A precondition describes the function's expectation of its arguments and/or the state of other
 * objects upon entry into the function and is usually placed at the start of a function body.
 *
 ...
 *
 * A postcondition is a condition that a function should ensure for the return value and/or the
 * state of objects upon exit from the function and is usually placed immediately before returning
 * control to the caller of the function.
 *
 * \note The postcondition is ignored if the contract checking build mode is `OFF`.
 *
 * \param cond an expression, that specifies the predicate of the contract.
 */
 ...
 *
 * An assertion is a condition that should be satisfied where it appears in a function body and can
 * be placed anywhere within the body of a function.
 *
 ...
 * objects upon entry into the function and is usually placed at the start of a function body.
 *
 * \note The precondition is enforced only if the contract checking build mode is `AUDIT`.
 *
 * \param cond an expression, that specifies the predicate of the contract.
 */
 ...
 * control to the caller of the function.
 *
 * \note The postcondition is enforced only if the contract checking build mode is `AUDIT`.
 *
 * \param cond an expression, that specifies the predicate of the contract.
 */
 ...
 * be placed anywhere within the body of a function.
 *
 * \note The assertion is enforced only if the contract checking build mode is `AUDIT`.
 *
 * \param cond an expression, that specifies the predicate of the contract.
 */
