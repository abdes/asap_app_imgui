...
//-----------------------------------------------------------------------------
// A) You may edit imconfig.h (and not overwrite it when updating imgui, or maintain a patch/branch
// with your modifications to imconfig.h) B) or add configuration directives in your own file and
// compile with #define IMGUI_USER_CONFIG "myfilename.h" If you do so you need to make sure that
// configuration settings are defined consistently _everywhere_ dear imgui is used, which include
// the imgui*.cpp files but also _any_ of your code that uses imgui. This is because some
// compile-time options have an affect on data structures. Defining those options in imconfig.h will
// ensure every compilation unit gets to see the same data structure layouts. Call
...
//default allocators calling malloc()/free(). You will need to call ImGui::SetAllocatorFunctions().
...
//---- Define constructor and implicit cast operators to convert back<>forth from your math types
//and ImVec2/ImVec4.
